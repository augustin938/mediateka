...
- 💬 **Чаты между друзьями**: переписка в реальном времени (SSE), шаринг элементов коллекции, реакции и удаление сообщений
...
│   │   ├── recommendations/ # Рекомендации
│   │   ├── chat/          # Сообщения, реакции, realtime stream
...
| GET/POST | `/api/chat/messages` | Получить историю чата / отправить сообщение или shared-item |
| DELETE | `/api/chat/messages/:id` | Удалить своё сообщение |
| POST/DELETE | `/api/chat/messages/:id/reactions` | Добавить / убрать реакцию |
| GET | `/api/chat/stream?with=:userId` | Realtime поток событий чата (SSE) |
...
chat_conversation — Диалоги между друзьями
chat_message    — Сообщения чата (text/share)
chat_message_reaction — Реакции на сообщения
