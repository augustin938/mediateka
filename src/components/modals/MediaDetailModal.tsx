...
export default function MediaDetailModal({
  ...
  const [selectedStatus, setSelectedStatus] = useState<
    "WANT" | "IN_PROGRESS" | "COMPLETED" | "DROPPED"
  >("WANT");
        ...
        <div className="flex flex-col sm:flex-row gap-0 sm:gap-6 p-6">
          ...
          <div className="flex-1 space-y-4 mt-4 sm:mt-0">
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              {item.externalRating && (
                <span className="text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2 py-1 rounded-lg">
                  ★ {item.externalRating}
                </span>
              )}
              {item.director && (
                <span className="text-muted-foreground bg-muted/50 px-2 py-1 rounded-lg">
                  Реж.: {item.director}
                </span>
              )}
              {item.author && (
